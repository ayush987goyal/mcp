...
def test_get_profile_arn():
    ...
    with patch.dict(os.environ, {'AWS_REGION': 'us-west-2', 'AWS_ACCOUNT_ID': '123456789012'}):
        assert (
            get_profile_arn() == 'arn:aws:bedrock:us-west-2:123456789012:data-automation-profile/us.data-automation-v1'
        )
...
def test_get_bedrock_data_automation_runtime_client():
    ...
    with patch('boto3.client') as mock_client:
        get_bedrock_data_automation_runtime_client()
        mock_client.assert_called_once_with('bedrock-data-automation-runtime', region_name=get_region())
...
async def test_list_projects():
    ...
    mock_client.list_data_automation_projects.return_value = {'projects': [{'name': 'test-project'}]}
...
async def test_upload_to_s3():
    ...
    with patch.dict(os.environ, {'AWS_BUCKET_NAME': 'test-bucket'}):
        with patch('awslabs.aws_bedrock_data_automation_mcp_server.helpers.Path.exists', return_value=True):
            with patch('builtins.open', mock_open(read_data=b'test data')):
                with patch(
                    'awslabs.aws_bedrock_data_automation_mcp_server.helpers.uuid.uuid4', return_value='test-uuid'
                ):
                    with patch(
...
async def test_upload_to_s3_file_not_exists():
    ...
    with patch.dict(os.environ, {'AWS_BUCKET_NAME': 'test-bucket'}):
        with patch('awslabs.aws_bedrock_data_automation_mcp_server.helpers.Path.exists', return_value=False):
            with pytest.raises(ValueError, match='Asset at path /path/to/asset.pdf does not exist'):
                await upload_to_s3('/path/to/asset.pdf')
...
async def test_download_from_s3():
    ...
    with patch('awslabs.aws_bedrock_data_automation_mcp_server.helpers.get_s3_client', return_value=mock_client):
        result = await download_from_s3('s3://test-bucket/path/to/file.json')
        assert result == {'key': 'value'}
        mock_client.get_object.assert_called_once_with(Bucket='test-bucket', Key='path/to/file.json')
...
async def test_invoke_data_automation_and_get_results():
    ...
    with patch(
        ...
        with patch.dict(os.environ, {'AWS_BUCKET_NAME': 'test-bucket', 'AWS_ACCOUNT_ID': '123456789012'}):
            with patch(
                'awslabs.aws_bedrock_data_automation_mcp_server.helpers.get_bedrock_data_automation_runtime_client'
            ) as mock_get_runtime:
                mock_runtime = MagicMock()
                mock_runtime.invoke_data_automation_async.return_value = {'invocationArn': 'test-invocation-arn'}
                ...
                mock_runtime.get_data_automation_status.side_effect = [
                    {'status': 'InProgress'},
                    {'status': 'Success', 'outputConfiguration': {'s3Uri': 's3://test-bucket/mcp/test-output'}},
                ]
                ...
                with patch('asyncio.sleep', new=AsyncMock()):
                    ...
                    with patch(
                        ...
                        result = await invoke_data_automation_and_get_results('/path/to/asset.pdf', 'test-project-arn')
                        ...
                        mock_runtime.invoke_data_automation_async.assert_called_once_with(
                            ...
                            dataAutomationConfiguration={'dataAutomationProjectArn': 'test-project-arn'},
        ...
        with patch.dict(os.environ, {'AWS_BUCKET_NAME': 'test-bucket', 'AWS_ACCOUNT_ID': '123456789012'}):
            with patch(
                ...
                with patch(
                    ...
                    assert result == {'standardOutput': {'standard': 'output'}, 'customOutput': {'custom': 'output'}}
...
async def test_invoke_data_automation_and_get_results_no_profile_arn():
    ...
    with patch(
        ...
        with patch.dict(os.environ, {'AWS_BUCKET_NAME': 'test-bucket'}, clear=True):
            with pytest.raises(ValueError, match='Could not determine profile ARN. Make sure AWS_ACCOUNT_ID is set.'):
    ...
    with patch(
        ...
        with patch.dict(os.environ, {'AWS_BUCKET_NAME': 'test-bucket', 'AWS_ACCOUNT_ID': '123456789012'}):
            with patch(
                ...
                mock_runtime.get_data_automation_status.return_value = {'status': 'Success', 'outputConfiguration': {}}
                mock_get_runtime.return_value = mock_runtime
            ...
            with patch(
                ...
                with patch(
                    ...
                    new=AsyncMock(
                        return_value={
                            'output_metadata': [
                                {'segment_metadata': [{'standard_output_path': None, 'custom_output_path': None}]}
                            ]
                        }
                    ),
                ):
        ...
        with patch.dict(os.environ, {'AWS_BUCKET_NAME': 'test-bucket', 'AWS_ACCOUNT_ID': '123456789012'}):
            with patch(
                ...
                with patch(
                    ...
                    result = await invoke_data_automation_and_get_results('/path/to/asset.pdf')
                    assert result == {'standardOutput': {'standard': 'output'}, 'customOutput': None}
